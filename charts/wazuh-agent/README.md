...
The minimal configuration required to run the wazuh agent is to specify the Wazuh server ip address in the `joinManagerMasterHost` and `joinManagerWorkerHost` values.
...
| `agent.image.registry`                                      | Wazuh Agent image registry                                                                                                                     | `""`                 |
| `agent.image.repository`                                    | Wazuh Agent image repository                                                                                                                   | `avisto/wazuh-agent` |
...
| `agent.configuration.joinManagerWorkerHost`                 | Ip address or Domain name of Wazuh worker for agent connection, if using ALL in One installation the same value as for "joinManagerMasterHost" | `""`                 |
...
| `agent.configuration.joinManagerUser.secretKeyRef.name`     | Name of the Kubernetes secret containing the Wazuh API credentials                                                                             | `wazuh-api-cred`     |
| `agent.configuration.joinManagerUser.secretKeyRef.key`      | Name of the key in the Kubernetes secret containing the Wazuh API username                                                                     | `username`           |
| `agent.configuration.joinManagerPassword.secretKeyRef.name` | Name of the Kubernetes secret containing the Wazuh API credentials                                                                             | `wazuh-api-cred`     |
| `agent.configuration.joinManagerPassword.secretKeyRef.key`  | Name of the key in the Kubernetes secret containing the Wazuh API password                                                                     | `password`           |
